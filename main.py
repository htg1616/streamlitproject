...
def graph(ticker, start, end, data):
    ...
    with st_graph_col2:
        one_date = pd.to_datetime(
            st.date_input(label='언제 주가를 알려줄까?', value=start, min_value=start, max_value=end))
...
def compare(ticker, start, end, data):
    compare_tickers = st.text_input(f'{ticker}와 비교할 ETF 또는 주식의 종목코드를 입력하세요', 'QQQ' if ticker == 'SPY' else 'SPY').split()
    fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, )
    ax1.plot(data.index, data['Close'], label=ticker)

    data['relative_Close'] = data['Close']/st.session_state['initialClose']
    ...
    ax1.legend()
    ax2.legend()
    ax1.set_title(f'{ticker} vs {", ".join(compare_tickers)} 종가 비교')
    ax2.set_title(f'{ticker} vs {", ".join(compare_tickers)} 종가 추이 비교')
...
with st.sidebar:
    st.title('ETF 투자 도우미')
    st.radio('페이지', ['주가 그래프', '비교', '백테스트'], index=None, key='page')
    st.divider()
    ticker, start, end, data = main_information()

page = st.session_state['page']
if page == '주가 그래프':
    graph(ticker, start, end, data)
elif page == '비교':
    compare(ticker, start, end, data)
elif page == '백테스트':
